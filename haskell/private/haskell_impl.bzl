...
def haskell_import_impl(ctx):
    ...
    import_info = HaskellImportHack(
        ...
        # static_libraries.
        dynamic_libraries = depset([ctx.file.shared_library], order = "topological", transitive = [
            dep[HaskellImportHack].dynamic_libraries for dep in ctx.attr.deps
        ]),
        static_libraries = depset([ctx.file.static_library], order = "topological", transitive = [
            dep[HaskellImportHack].static_libraries for dep in ctx.attr.deps
        ]),
        headers = depset(ctx.files.hdrs, transitive = [
            dep[HaskellImportHack].headers for dep in ctx.attr.deps
        ]),
        includes = depset(ctx.attr.includes, transitive = [
            dep[HaskellImportHack].includes for dep in ctx.attr.deps
        ]),
        linkopts = ctx.attr.linkopts + [
            opt for dep in ctx.attr.deps for opt in dep[HaskellImportHack].linkopts
        ],
    )
